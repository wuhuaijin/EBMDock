...
class CrossAttentionLayer(torch.nn.Module):
    def __init__(self,
        ...
        self.proj_q = LinearRS(irreps_attn_heads, self.irreps_node_output)
        self.proj_k = LinearRS(irreps_attn_heads, self.irreps_node_output)
        self.proj_v = LinearRS(irreps_attn_heads, self.irreps_node_output)
        self.fc = LinearRS(irreps_attn_heads, self.irreps_node_output)
        self.vec2heads = self.vec2heads_value = Vec2AttnHeads(self.irreps_head, num_heads)
        self.heads2vec = AttnHeads2Vec(irreps_head)


    def forward(self, src_h, dst_h, src_num_nodes, dst_num_nodes):

        src_h_list = torch.split(src_h, src_num_nodes)
        dst_h_list = torch.split(dst_h, dst_num_nodes)
        h_msg = []
        for idx in range(len(src_num_nodes)):
            src_hh = src_h_list[idx]
            dst_hh = dst_h_list[idx]
            query = self.proj_q(dst_hh)
            dim = query.shape[-1]
            query = self.vec2heads(query)
            query = query.view(-1, self.num_heads, dim // self.num_heads).permute(1, 0, 2)

            key = self.proj_k(src_hh)
            key = self.vec2heads(key)
            key = key.view(-1, self.num_heads, dim // self.num_heads).permute(1, 0, 2)

            value = self.proj_v(src_hh)
            value = self.vec2heads(value)
            value = value.view(-1, self.num_heads, dim // self.num_heads).permute(1, 0, 2)

            attn = torch.matmul(query, key.transpose(1, 2)) / ((dim // self.num_heads) ** 0.5)
...
class TransBlock(torch.nn.Module):
    ...
    def __init__(self,
        ...
        drop_path_rate=0.0,
        irreps_mlp_mid=None,
        norm_layer='layer',cross_attention=False):
        ...
        if cross_attention:
            self.crossga = CrossAttentionLayer(irreps_node_input=self.irreps_node_input, 
                irreps_node_output=self.irreps_node_input,
                irreps_head=self.irreps_head, 
                num_heads=self.num_heads)
    ...
    def forward(self, node_input1, node_attr1, edge_src1, edge_dst1, edge_attr1, edge_scalars1, 
        batch1, node_input2, node_attr2, edge_src2, edge_dst2, edge_attr2, edge_scalars2, batch2, **kwargs):
        
        node_output1 = node_input1
        node_features1 = node_input1
        node_features1 = self.norm_1(node_features1, batch=batch1)
        node_features1 = self.ga(node_input=node_features1, 
            node_attr=node_attr1, 
            edge_src=edge_src1, edge_dst=edge_dst1, 
            edge_attr=edge_attr1, edge_scalars=edge_scalars1,
            batch=batch1)
        
        node_output2 = node_input2
        node_features2 = node_input2
        node_features2 = self.norm_2(node_features2, batch=batch2)
        node_features2 = self.ga(node_input=node_features2, 
            node_attr=node_attr2, 
            edge_src=edge_src2, edge_dst=edge_dst2, 
            edge_attr=edge_attr2, edge_scalars=edge_scalars2,
            batch=batch2)

        if self.cross_attention:
            num_nodes_1 = torch.bincount(batch1).tolist()
            num_nodes_2 = torch.bincount(batch2).tolist()

            node_features1_new = self.crossga(node_features2, node_features1, num_nodes_2, num_nodes_1)
            node_features2 = self.crossga(node_features1, node_features2, num_nodes_1, num_nodes_2)
            node_features1 = node_features1_new
        
        
        if self.drop_path is not None:
            node_features1 = self.drop_path(node_features1, batch1)
        node_output1 = node_output1 + node_features1
        
        node_features1 = node_output1
        node_features1 = self.norm_2(node_features1, batch=batch1)
        node_features1 = self.ffn(node_features1, node_attr1)
        if self.ffn_shortcut is not None:
            node_output1 = self.ffn_shortcut(node_output1, node_attr1)
        
        if self.drop_path is not None:
            node_features2 = self.drop_path(node_features2, batch2)
        node_output2 = node_output2 + node_features2
        
        if self.drop_path is not None:
            node_features2 = self.drop_path(node_features2, batch2)
        node_output2 = node_output2 + node_features2
        
        node_features2 = node_output2
        node_features2 = self.norm_2(node_features2, batch=batch2)
        node_features2 = self.ffn(node_features2, node_attr2)
        if self.ffn_shortcut is not None:
            node_output2 = self.ffn_shortcut(node_output2, node_attr2)
